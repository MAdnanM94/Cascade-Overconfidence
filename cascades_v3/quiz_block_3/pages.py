...
class PreDecision(Page):
    ...
    def before_next_page(self):
        current_sequence = self.player.participant.vars['sequences_order_4'][self.group.order - 1]
        true_state = self.session.vars['true_state_Red_4'][current_sequence - 1]
...
class OrderFinish(WaitPage):
    def after_all_players_arrive(self):
        ...
        if self.group.id_in_subsession == 1:
            self.group.red_hist_dump = str(self.session.vars['Red_hist_4_1'])
        else:
            self.group.red_hist_dump = str(self.session.vars['Red_hist_4_2'])
