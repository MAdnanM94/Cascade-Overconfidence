    ...
    def initialization(self):
        ...
        if self.round_number == 1:
            ...
            for p in self.get_players():
                ...
                p.participant.vars['paying_round_4'] = paying_round
                p.participant.vars['paying_decision_4'] = paying_decision
                p.participant.vars['paying_guess_4'] = paying_guess

        for p in self.get_players():
            p.paying_round = p.participant.vars['paying_round_4']
            p.paying_decision = p.participant.vars['paying_decision_4']
            p.paying_guess = p.participant.vars['paying_guess_4']
        ...
        self.session.vars['true_state_Red_4'] = [r <= Constants.prior_red for r in state_r]
        ...
        self.session.vars['Red_hist_4_1'] = [[False for i in range(Constants.players_per_group)] for j in
                                             range(Constants.spell_length)]
        self.session.vars['Red_hist_4_2'] = [[False for i in range(Constants.players_per_group)] for j in
                                             range(Constants.spell_length)]
        ...
        for g in self.get_groups():
            ...
            g.order = 1
            # set order = 1
            g.true_state_red_dump = str(self.session.vars['true_state_Red_4'])
...
class Group(BaseGroup):
    ...
    def set_sequence(self):
        ...
        j = 0
        for p in self.get_players():
            p.participant.vars['sequences_order_4'] = [row[j] for row in sequences]

            [p.sequence1, p.sequence2, p.sequence3, p.sequence4, p.sequence5, p.sequence6,
             ] = p.participant.vars['sequences_order_4']
    ...
    def record_history(self):
        ...
        for p in self.get_players():
            psequence = p.participant.vars['sequences_order_4'][self.order - 1]
        ...
        if self.id_in_subsession == 1:
            self.session.vars['Red_hist_4_1'][self.order - 1] = current_hist
        else:
            self.session.vars['Red_hist_4_2'][self.order - 1] = current_hist
...
class Player(BasePlayer):
    ...
    def record_state(self):
        if self.group.order == 1:
            self.stateR1 = self.session.vars['true_state_Red_4'][self.sequence1 - 1]
        if self.group.order == 2:
            self.stateR2 = self.session.vars['true_state_Red_4'][self.sequence2 - 1]
        if self.group.order == 3:
            self.stateR3 = self.session.vars['true_state_Red_4'][self.sequence3 - 1]
        if self.group.order == 4:
            self.stateR4 = self.session.vars['true_state_Red_4'][self.sequence4 - 1]
        if self.group.order == 5:
            self.stateR5 = self.session.vars['true_state_Red_4'][self.sequence5 - 1]
        if self.group.order == 6:
            self.stateR6 = self.session.vars['true_state_Red_4'][self.sequence6 - 1]
    ...
    def vars_for_template_history(self):
        ...
        if self.group.id_in_subsession == 1:
            current_hist = self.session.vars['Red_hist_4_1']
        else:
            current_hist = self.session.vars['Red_hist_4_2']
    ...
    def decision_profit(self):
        ...
        self.participant.vars['guess_profit_4'] = self.guess_profit
    ...
    def quiz_profit_calc(self):
        ...
        self.participant.vars['quiz_profit_4'] = self.quiz_profit  # payoff from quiz
        self.participant.vars['own_score_4'] = self.score  # own score
        self.participant.vars['other_score_4'] = self.other_score  # random participant's score
        self.participant.vars['own_question_4'] = self.question_own  # question chosen for belief elicitation payment
        self.participant.vars[
            'other_question_4'] = self.question_other  # question chosen for belief elicitation payment