...
class Group(BaseGroup):
    ...
    def set_chain(self):
        ...
        j = 0
        for p in self.get_players():
            p.participant.vars['chains_order_2'] = [row[j] for row in chains]

            [p.chain1, p.chain2, p.chain3, p.chain4, p.chain5, p.chain6,
             ] = p.participant.vars['chains_order_2']
    ...
    def record_history(self):
        ...
        for p in self.get_players():
            pchain = p.participant.vars['chains_order_2'][self.order - 1]
...
class Player(BasePlayer):
    ...
    def record_state(self):
        if self.group.order == 1:
            self.stateR1 = self.session.vars['true_state_Red_2'][self.chain1 - 1]
        if self.group.order == 2:
            self.stateR2 = self.session.vars['true_state_Red_2'][self.chain2 - 1]
        if self.group.order == 3:
            self.stateR3 = self.session.vars['true_state_Red_2'][self.chain3 - 1]
        if self.group.order == 4:
            self.stateR4 = self.session.vars['true_state_Red_2'][self.chain4 - 1]
        if self.group.order == 5:
            self.stateR5 = self.session.vars['true_state_Red_2'][self.chain5 - 1]
        if self.group.order == 6:
            self.stateR6 = self.session.vars['true_state_Red_2'][self.chain6 - 1]
